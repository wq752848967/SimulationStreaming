...
public class DataSplit {
    public static void main(String[] args) {
        ...
        Dataset<Row> ds = session_java.read().option("header","true").option("delimiter","|").csv("hdfs://192.168.35.55:9000/flok/4665/DWFReadMySQL-1530083324_475428cc-5f2c-4291-915b-e5c3759b9405_0.output");
        double[] s = {0.33,0.33,0.33};
...
//        session_java.sql("select * from filter_right,filter_left where  filter_left.period_start>=filter_right.new_timestamp and filter_left.period_end<=filter_right.new_timestamp").show();
          dss[0].write().option("header","true").option("delimiter","|").mode(SaveMode.Overwrite).csv("hdfs://192.168.35.55:9000/flok/sim/sp2/left_31.csv");
          dss[1].write().option("header","true").option("delimiter","|").mode(SaveMode.Overwrite).csv("hdfs://192.168.35.55:9000/flok/sim/sp2/left_32.csv");
          dss[2].write().option("header","true").option("delimiter","|").mode(SaveMode.Overwrite).csv("hdfs://192.168.35.55:9000/flok/sim/sp2/left_33.csv");
    }
}
